...
/*
const getContrastText = (backgroundColor, contrast=21) => {
	let opacityMap, textColor;
...
//const textColor = getContrastText(backgroundColor, 13);
//const textOverlayOnBackground = textColor.overlayOn(backgroundColor);
...
lum1 = textOverlaidBackground.luminance
lum2 = background.luminance
brightest = Math.max(lum1, lum2)
darkest = Math.min(lum1, lum2)
contrast = (brightest + 0.05) / (darkest + 0.05)

provide background, text color, and contrast
const getContrastText = (backgroundColor) => {
	const [R, G, B] = backgroundColor
	const brightness = (299*R + 587*G + 114*B) / 1000
	return (bcBrightness > 125) ? color('0, 0, 0') : color('255, 255, 255');
}

const textOpacity = (backgroundColor, textColor, contrast) => {
